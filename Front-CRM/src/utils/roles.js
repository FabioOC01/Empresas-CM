...
export function isMarketingPure(entity) {
    const roles = getEffectiveRoles(entity);
    return roles.includes('Marketing') && !roles.some((role) => ['Ventas', 'Retail', 'Corporativo'].includes(role));
}
